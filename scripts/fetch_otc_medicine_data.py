"""
hosppass 안전상비의약품 판매업소(편의점 등) 데이터 수집 스크립트
행정안전부_건강_안전상비의약품 판매업소 표준데이터(전국 지자체 인허가 정보 취합)를
localdata.go.kr에서 CSV로 내려받아 data/otc_medicine_stores.json으로 정규화한다.
...
실행: python scripts/fetch_otc_medicine_data.py
"""
import csv
import io
import sys
...
DATA_DIR = ROOT / "data"
RAW_PATH = DATA_DIR / "otc_medicine_raw.csv"
OUT_PATH = DATA_DIR / "otc_medicine_stores.json"

DOWNLOAD_URL = "https://file.localdata.go.kr/file/download/over_the_counter_medicine_stores/info"
DETAIL_URL = "https://file.localdata.go.kr/file/over_the_counter_medicine_stores/info"
...
def download():
    ...
    if len(resp.content) < 100_000:
        raise SystemExit(
            ...
            "localdata.go.kr 다운로드 URL이 바뀌었을 수 있음 — 브라우저로 "
            f"{DETAIL_URL} 방문 후 '전국 파일 다운로드' 버튼의 실제 요청을 다시 확인할 것."
        )
...
def parse():
    # Content-Type은 UTF-8이라고 표기돼 있지만 실제로는 CP949(EUC-KR)로 인코딩되어 있음
